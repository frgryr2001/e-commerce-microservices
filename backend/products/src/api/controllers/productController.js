...
const IMGBB_API_KEY = process.env.IMGBB_API_KEY || '61a7cb2f7042d5741f20bc5142ce9791';
...
class ProductController {
	...
	async getProductById(req, res) {
		try {
			...
			console.error(err.message);
			res.status(400).json({ status: 'Thất bại', message: 'Không tìm thấy sản phẩm' });
		}
	}
	...
	async deleteProduct(req, res) {
		try {
			...
			const product_options = await ProductOption.find({ product_id: req.params.id });
			const product_images = await ProductImage.find({ product_id: req.params.id });
			for (let i = 0; i < product_options.length; i++) {
